...
const TMP_DIR = '/tmp/day02-screenshots';
...
(async () => {
  ...
  for (const card of cards) {
    ...
    try {
      ...
      const tmpPath = path.join(TMP_DIR, card.output);
      await page.screenshot({
        path: tmpPath,
        clip: { x: 0, y: 0, width: 1024, height: 1536 }
      });
      console.log(`  -> Tmp saved: ${tmpPath}`);
      
      // Copy to destination using fs
      const destPath = path.join(DIR, card.output);
